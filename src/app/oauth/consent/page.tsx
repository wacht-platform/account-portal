...
function SkeletonLine({ w, h = "h-4" }: { w: string; h?: string }) {
  return (
    <div
      ...
      style={{
        background: "var(--color-background-hover, var(--color-accent, var(--accent)))",
      }}
    />
  );
}
            ...
            {[1, 2, 3, 4].map((i) => (
              <div key={i} className="flex items-start gap-3">
                <div
                  ...
                  style={{ background: "var(--color-primary-background)" }}
                />
...
export default function OAuthConsentPage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      if (res.ok) {
        ...
        setSelectedResource(
          (payload?.resource_options?.[0]?.value as string | undefined) || "",
        );
        setError(null);
        setLoading(false);
        return;
  ...
  const submitUrl = useMemo(() => {
    ...
    if (deployment.mode === "staging") {
      const devSession = (localStorage.getItem("__dev_session__") || "").trim();
      if (devSession) {
                ...
                <>
                  ...
                  <div className="px-6 py-4 space-y-2">
                    ...
                    <div className="relative">
                      ...
                      >
                        {context.resource_options?.map((option) => (
              ...
              <div className="px-6 py-4 space-y-3">
                <div className="flex items-center gap-3">
                  <form method="POST" action={submitUrl} className="flex-1">
                    ...
                    <SecondaryActionButton>Deny</SecondaryActionButton>
                  </form>
                  <form method="POST" action={submitUrl} className="flex-1">
                    ...
                    <input
                      ...
                      value={selectedResource}
                    />
                    <PrimaryActionButton>Allow access</PrimaryActionButton>
                  </form>
                </div>
